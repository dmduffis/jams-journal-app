...
function normalizeHighlight(h) {
  ...
  return {
    ...
    text: h.text ?? "",
    startOffset: h.startOffset ?? h.start_offset ?? 0,
    endOffset: h.endOffset ?? h.end_offset ?? 0,
    createdAt: h.createdAt ?? h.created_at ?? h.created,
  };
}

/**
 * Create a highlight. Body: { articleId, text, startOffset?, endOffset? }.
 */
export async function addHighlight(articleId, { text, startOffset, endOffset }) {
  const userId = await getUserId();
  const headers = await getAuthHeaders();
  const res = await fetch(`${BASE_URL}/users/${userId}/highlights`, {
    method: "POST",
    headers,
    body: JSON.stringify({
