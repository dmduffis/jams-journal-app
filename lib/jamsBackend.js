...
/** Bookmark/save count for an article (no auth). Optional backend endpoint. */
export async function getArticleBookmarksCount(articleId) {
  try {
    const res = await fetch(`${BASE_URL}/articles/${encodeURIComponent(String(articleId))}/bookmarks/count`);
    if (!res.ok) return 0;
    const data = await res.json();
    const n =
      typeof data === "number"
